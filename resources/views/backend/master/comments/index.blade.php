...
<div class="card">
    <div class="card-header with-elements">
        <h5 class="card-header-title mt-1 mb-0">Comment List</h5>
    ...
    <div class="table-responsive">
        <table id="user-list" class="table card-table table-striped table-bordered table-hover">
            ...
            <tbody>
                @if ($data['comment']->total() == 0)
                    <tr>
                        <td colspan="7" align="center">
                            <i>
                                <strong style="color:red;">
                                @if (Request::get('l') || Request::get('f') || Request::get('q'))
                                ! Comments not found :( !
                                @else
                                ! Comments not record !
                                @endif
                ...
                @foreach ($data['comment'] as $item)
                    <tr>
                        ...
                        <td>
                            ...
                            <a href="javascript:void(0);" onclick="$(this).find('form').submit();" class="badge badge-{{ config('custom.label.flags.'.$item->flags)['color'] }}"
                                title="Click to change comment flags">
                        ...
                        <td>
                            @can ('comment_delete')
                            <button type="button" class="btn btn-danger icon-btn btn-sm js-delete" title="delete comment"
                                data-id="{{ $item->id }}">
                    ...
                    <tr>
                        ...
                        <td>
                            @can ('comment_edit')
                            <a href="javascript:void(0);" onclick="$(this).find('form').submit();" class="badge badge-{{ config('custom.label.flags.'.$reply->flags)['color'] }}"
                                title="Click to change comment reply flags">
                                {{ config('custom.label.flags.'.$reply->flags)['title'] }}
                                <form action="{{ route('comment.reply.flags', ['id' => $reply->id]) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                </form>
                            </a>
                            @else
                            <span class="badge badge-{{ config('custom.label.flags.'.$reply->flags)['color'] }}">{{ config('custom.label.flags.'.$reply->flags)['title'] }}</span>
                            @endcan
                        </td>
                        <td>{{ $reply->created_at->format('d F Y (H:i A)') }}</td>
                        <td>{{ $reply->updated_at->format('d F Y (H:i A)') }}</td>
                        <td>
                            @can ('comment_delete')
                            <button type="button" class="btn btn-danger icon-btn btn-sm js-delete-2" title="delete comment reply"
                                data-id="{{ $reply->id }}">
                                <i class="las la-trash-alt"></i>
                            </button>
                            @else
    ...
    <div class="card-footer">
        <div class="row align-items-center">
            <div class="col-lg-6 m--valign-middle">
                Showing : <strong>{{ $data['comment']->firstItem() }}</strong> - <strong>{{ $data['comment']->lastItem() }}</strong> of
                <strong>{{ $data['comment']->total() }}</strong>
            </div>
            <div class="col-lg-6 m--align-right">
                {{ $data['comment']->onEachSide(1)->links() }}
            </div>
        </div>
    </div>
</div>
    ...
    $(document).ready(function () {
        $('.js-delete').on('click', function () {
            ...
            Swal.fire({
                ...
                preConfirm: () => {
                    return $.ajax({
                        url: '/admin/management/comment/' + id,
                        method: 'DELETE',
                        headers: {
        ...
        $('.js-delete-2').on('click', function () {
            ...
            Swal.fire({
                ...
                preConfirm: () => {
                    return $.ajax({
                        url: '/admin/management/comment/' + id + '/reply',
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
                ...
                if (response.value.success) {
                    Swal.fire({
                        type: 'success',
                        text: 'comment reply successfully deleted'
                    }).then(() => {
                        window.location.reload();
                    })
                } else {
