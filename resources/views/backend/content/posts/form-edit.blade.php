...
<div class="card">
    <div class="list-group list-group-flush account-settings-links flex-row">
        ...
        @endif
        @if ($data['post']->field->count() > 0)
        <a class="list-group-item list-group-item-action" data-toggle="list" href="#field">Addon Field</a>
        @endif
    </div>
    <div class="card-body">
        <form action="{{ route('post.update', ['sectionId' => $data['post']->section_id, 'id' => $data['post']->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="tab-content">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" class="form-control mb-1 gen_slug @error('title_'.$lang->iso_codes) is-invalid @enderror"
                                            lang="{{ $lang->iso_codes }}" name="title_{{ $lang->iso_codes }}" value="{{ old('title_'.$lang->iso_codes, $data['post']->fieldLang('title', $lang->iso_codes)) }}" placeholder="Enter title...">
                ...
                <div class="tab-pane fade" id="setting">
                    <div class="card-body">
                            ...
                            <div class="col-sm-10">
                                <select class="selectpicker custom-select" name="category_id" data-style="btn-default">
                                    ...
                                    @foreach ($data['categories'] as $category)
                                        <option value="{{ $category->id }}" {{ old('category_id', $category->id) == $data['post']->category_id ? 'selected' : '' }}>{!! $category->fieldLang('name') !!}</option>
                                    @endforeach
                                </select>
                                @error('category_id')
                            ...
                            <div class="col-sm-10">
                                <select class="custom-select" name="publish" data-style="btn-default">
                                    @foreach (config('custom.label.publish') as $key => $publish)
                                        <option value="{{ $key }}" {{ (old('publish') == ''.$key.'') == $data['post']->publish ? 'selected' : '' }}>
                                            {{ $publish['title'] }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-sm-10">
                                <div class="input-group">
                                    <input type="text" class="datetime-picker form-control @error('created_at') is-invalid @enderror" name="created_at"
                                        value="{{ old('created_at', $data['post']->created_at) }}" placeholder="enter publish time...">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                                <label class="custom-control custom-checkbox m-0">
                                    <input type="checkbox" class="custom-control-input" name="public" value="1" {{ old('public', $data['post']->public) ? 'checked' : 'checked' }}>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                                <label class="custom-control custom-checkbox m-0">
                                    <input type="checkbox" class="custom-control-input" name="is_widget" value="1" {{ old('is_widget', $data['post']->is_widget) ? 'checked' : '' }}>
                                    <span class="custom-control-label ml-4">YES</span>
                                </label>
                            </div>
                        </div>
                            ...
                            <div class="col-sm-10">
                                <div class="input-group">
                                    <input type="text" class="form-control" id="image1" aria-label="Image" aria-describedby="button-image" name="cover_file"
                                            value="{{ old('cover_file', $data['post']->cover['file_path']) }}" placeholder="browse cover...">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                                <textarea class="form-control mb-1" name="meta_keywords" placeholder="Separated by comma (,)">{{ old('meta_keywords', $data['post']->meta_data['keywords']) }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="tab-pane fade" id="files">
                    <div class="card-body">
                        ...
                        <div id="delete-file-{{ $file->id }}">
                            <div class="form-group row">
                                <div class="col-md-2 text-md-right">
                                    <label class="col-form-label text-sm-right">Oldes File</label>
                                </div>
                                <div class="col-md-9">
                                    <div class="input-group">
                                        <input class="form-control" type="text" value="{{ $file->file }}" placeholder="enter file..." readonly>
                                        <div class="input-group-append">
                                            <button class="btn btn-primary" type="button"><i class="las la-file"></i></button>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-1">
                                    <div class="col-md-1 text-center">
                                        <button type="button" data-sectionid="{{ $data['post']->section_id }}" data-id="{{ $file->id }}"  id="delete-file" class="btn icon-btn btn-danger btn-sm"><span class="las la-times"></span></button>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="tab-pane fade" id="profile">
                    <div class="card-body">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                                <input type="{{ $valP['type'] }}" class="form-control mb-1 @error('profile_'.$valP['name']) is-invalid @enderror" name="profile_{{ $valP['name'] }}" value="{{ old('profile_'.$valP['name'], $data['post']->profile->field[$valP['name']]) }}" placeholder="Enter {{ $valP['label'] }}...">
                ...
                <div class="tab-pane fade" id="field">
                    <div class="card-body">
                        @foreach ($data['post']->field as $field)
                        @include('backend.master.field.input', ['module' => 'post'])
                        @endforeach
                    </div>
                </div>
                @endif
...
<script>
    ...
    $(document).on('click', '#delete-file', function() {
        ...
        var id = $(this).attr("data-id");
        $.ajax({
            url : '/admin/management/section/'+ section_id +'/post/'+ id + '/file',
            method: 'DELETE',
            success:function() {
