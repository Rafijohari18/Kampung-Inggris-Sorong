...
<div class="card">
    @if (Request::get('is_trash') == 'yes')
        @include('backend.languages.trash')
    @else
        @include('backend.languages.list')
    @endif
    <div class="card-footer">
        <div class="row align-items-center">
            <div class="col-lg-6 m--valign-middle">
                Showing : <strong>{{ $data['languages']->firstItem() }}</strong> - <strong>{{ $data['languages']->lastItem() }}</strong> of
...
<script>
    ...
    $('.modal-delete').click(function() {
        ...
        $('.modal-body .id').attr('data-id', id);
        $('.text-large #name').text(name);
    ...
    $(document).ready(function () {
        ...
        $('.soft-delete').on('click', function () {
            ...
            Swal.fire({
                title: "Are you sure?",
                text: "data will be moved to the trash!",
                type: "warning",
                ...
                preConfirm: () => {
                    return $.ajax({
                        url: '/admin/management/language/' + id + '/soft',
                        method: 'DELETE',
                        headers: {
                ...
                if (response.value.success) {
                    Swal.fire({
                        type: 'success',
                        text: 'language successfully move to trash'
                    }).then(() => {
        ...
        $('.permanent-delete').on('click', function () {
            ...
            Swal.fire({
                ...
                preConfirm: () => {
                    return $.ajax({
                        url: '/admin/management/language/' + id + '/permanent?is_trash={{ Request::get('is_trash') }}',
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
    ...
    $('.restore').click(function(e) {
        ...
        Swal.fire({
        title: "Are you sure ?",
        text: "data will be returned to the list",
        type: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        confirmButtonText: 'Yes, restore!',
        cancelButtonText: "No, thanks you",
        }).then((result) => {
        if (result.value) {
