...
<div class="card">
    <div class="card-header with-elements">
        <h5 class="card-header-title mt-1 mb-0">Link List <a href="{{ config('custom.language.multiple') == true ? route('link.list', ['locale' => app()->getLocale()]) : route('link.list') }}" target="_blank"><i class="las la-external-link-alt"></i></a></h5>
        <div class="card-header-elements ml-auto">
            @can ('link_create')
            <a href="{{ route('link.create') }}" class="btn btn-success icon-btn-only-sm" title="add links">
        ...
        <table id="user-list" class="table card-table table-striped table-bordered table-hover">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-center">Total : <span class="badge badge-success">{{ $item->media->count() }}</span></td>
                    ...
                    <td>
                        @if (auth()->user()->can('link_edit') && $item->min('position') != $item->position)
                        ...
                        @endif
                        @if (auth()->user()->can('link_edit') && $item->max('position') != $item->position)
                    ...
                    <td>
                        @can('fields')
                        <a href="{{ route('field.form', ['id' => $item->id, 'module' => 'link']) }}" class="btn icon-btn btn-sm btn-secondary" title="setting addon field">
                        ...
                        @endcan
                        @can ('link_media')
                        <a href="{{ route('link.media', ['linkId' => $item->id]) }}" class="btn btn-sm icon-btn btn-info" title="list media">
                            <i class="las la-folder"></i>
                        </a>
                        @else
                        <button class="btn btn-sm icon-btn btn-secondary" title="you are not allowed to take this action" disabled><i class="las la-folder"></i></button>
                        @endcan
                        @can('link_edit')
                        ...
                        @endcan
                        @can('link_delete')
                        <a href="javascript:;" data-id="{{ $item->id }}" class="btn icon-btn btn-sm btn-danger js-delete" title="delete link">
