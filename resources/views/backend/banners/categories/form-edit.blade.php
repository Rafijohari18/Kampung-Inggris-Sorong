...
<div class="card">
    <div class="list-group list-group-flush account-settings-links flex-row">
        ...
        @if ($data['banner_category']->field->count() > 0)
        <a class="list-group-item list-group-item-action" data-toggle="list" href="#field">Addon Field</a>
        @endif
    </div>
    <div class="card-body">
        <form action="{{ route('banner.category.update', ['id' => $data['banner_category']->id]) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="tab-content">
                          ...
                          <div class="tab-pane fade {{ ($loop->first) ? 'active show' : '' }}" id="{{ $lang->iso_codes }}">
                            <div class="card-body pb-2">
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" class="form-control mb-1 gen_slug @error('name_'.$lang->iso_codes) is-invalid @enderror"
                                            lang="{{ $lang->iso_codes }}" name="name_{{ $lang->iso_codes }}" value="{{ old('name_'.$lang->iso_codes, $data['banner_category']->fieldLang('name')) }}" placeholder="Enter name...">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                        <textarea class="form-control tiny-mce" name="description_{{ $lang->iso_codes }}">{!! old('description_'.$lang->iso_codes, $data['banner_category']->fieldLang('description')) !!}</textarea>
                                    </div>
                                </div>
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="number" class="form-control mb-1 @error('list_limit') is-invalid @enderror" name="list_limit"
                                                value="{{ old('list_limit', $data['banner_category']->list_limit) }}" placeholder="Enter limit...">
                ...
                <div class="tab-pane fade" id="field">
                    <div class="card-body">
                        @foreach ($data['banner_category']->field as $field)
                        @include('backend.master.field.input', ['module' => 'banner_category'])
                        @endforeach
                    </div>
                </div>
                @endif
