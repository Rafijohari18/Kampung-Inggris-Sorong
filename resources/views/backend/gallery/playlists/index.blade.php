...
<div class="card">
    <div class="card-header with-elements">
        <h5 class="card-header-title mt-1 mb-0">Playlist List <a href="{{ config('custom.language.multiple') == true ? route('playlist.list', ['locale' => app()->getLocale()]) : route('playlist.list') }}" target="_blank"><i class="las la-external-link-alt"></i></a></h5>
        <div class="card-header-elements ml-auto">
            @can ('playlist_create')
            <a href="{{ route('gallery.playlist.create') }}" class="btn btn-success icon-btn-only-sm" title="add playlists">
                <i class="las la-plus"></i> <span>Playlist</span>
            </a>
            @endcan
        </div>
    </div>
        ...
        <table id="user-list" class="table card-table table-striped table-bordered table-hover">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-center">Total : <span class="badge badge-success">{{ $item->video->count() }}</span></td>
                    ...
                    <td class="text-center">
                        ...
                        <a href="javascript:void(0);" onclick="$(this).find('form').submit();" class="badge badge-{{ config('custom.label.publish.'.$item->publish)['color'] }}"
                            title="Click to change status playlist">
                    ...
                    <td>
                        @if (auth()->user()->can('playlist_edit') && $item->min('position') != $item->position)
                        ...
                        @endif
                        @if (auth()->user()->can('playlist_edit') && $item->max('position') != $item->position)
                    ...
                    <td>
                        @can ('playlist_video')
                        <a href="{{ route('gallery.playlist.video', ['playlistId' => $item->id]) }}" class="btn btn-sm icon-btn btn-info" title="list video">
                            <i class="las la-video"></i>
                        </a>
                        @else
                        <button class="btn btn-sm icon-btn btn-secondary" title="you are not allowed to take this action" disabled><i class="las la-video"></i></button>
                        @endcan
                        @can('playlist_edit')
                        <a href="{{ route('gallery.playlist.edit', ['id' => $item->id]) }}" class="btn icon-btn btn-sm btn-primary" title="edit playlist">
                        ...
                        @endcan
                        @can('playlist_delete')
                        <a href="javascript:;" data-id="{{ $item->id }}" class="btn icon-btn btn-sm btn-danger js-delete" title="delete playlist">
    ...
    $(document).ready(function () {
        $('.js-delete').on('click', function () {
            ...
            Swal.fire({
                ...
                preConfirm: () => {
                    return $.ajax({
                        url: '/admin/management/gallery/playlist/' + id,
                        method: 'DELETE',
                        headers: {
